...
<xsl:template match="rtmp">
    <table cellspacing="1" cellpadding="5">
        <tr bgcolor="#999999">
            <th>RTMP Statistics</th>
            <th>Link to Stream</th>
            <th># of RTMP clients</th>
            <th># of viewers</th>
...
<xsl:template match="live">
    <tr bgcolor="#aaaaaa">
        <td align="middle">
            <b><i>active stream info</i></b>
        </td>
        <td align="middle" bgcolor="#83e1a0">
            <b><i>click me to stream!</i></b>
        </td>
        <td align="middle">
...
<xsl:template match="stream">
    <tr valign="top">
        ...
        <td>
            <xsl:attribute name="bgcolor">
               <xsl:choose>
                  <xsl:when test="active">#83e1a0</xsl:when>
                  <xsl:otherwise>#999999</xsl:otherwise>
               </xsl:choose>
            </xsl:attribute>
            <!-- test: <xsl:value-of select="name"/> -->
            <xsl:variable name="myHREF">
               <xsl:value-of select="concat('https://[URL]/play/index.html?key=',name)"/>
            </xsl:variable>
            <A HREF="{$myHREF}" target="_blank"><xsl:value-of select="name"/></A>
        </td>
        <td align="middle">
            <xsl:value-of select="nclients"/>
        </td>
        <td align="middle">
            <xsl:variable name="myCountFile">
               <xsl:copy-of select="concat('/counts/',name,'.xml')"/>
            </xsl:variable>
            <!-- put viewers here -->
            <xsl:value-of select="document($myCountFile)//count"/>
        </td>
        <td>
